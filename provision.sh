...
aws ec2 create-key-pair --key-name MyKeyLab3 --query 'KeyMaterial' --output text > MyKeyLab3.pem \
&& chmod 400 MyKeyLab3.pem \
...
&& INST_ID=$(aws ec2 run-instances --image-id ami-0b0dcb5067f052a63 --count 1 --instance-type t2.micro --key-name MyKeyLab3 --security-group-ids $SG --user-data file://user_data.sh --query 'Instances[*].[InstanceId]' --output text) \
